# Requirements Archive: v1.0 Certificados Verificables
...
- [x] **INFRA-01**: PocketBase corre en el VPS con `--http=127.0.0.1:8090` (nunca expuesto en puerto público)
- [x] **INFRA-02**: nginx hace reverse proxy de `/api/*` y `/_/*` hacia PocketBase; sirve el SPA con `try_files`
- [x] **INFRA-03**: SSL/HTTPS configurado en nginx para el dominio de producción
- [x] **INFRA-04**: Colección `certificates` creada con todos los campos: `certificateCode`, `studentName`, `dni`, `university`, `degree`, `startDate`, `endDate`, `issueDate`, `score`, `technologies[]`, `competencies[]`, `description`, `supervisorName`, `status`
- [x] **INFRA-05**: `certificateCode` tiene índice UNIQUE en la base de datos
- [x] **INFRA-06**: API rules correctas: `listRule = @request.auth.id != ""`, `viewRule = ""`, `createRule = null`, `updateRule = null`, `deleteRule = null`
- [x] **INFRA-07**: Backup automático de la base de datos PocketBase configurado (cron diario)
- [x] **INFRA-08**: `VITE_POCKETBASE_URL` definido en `.env` y en `.env.production`; `src/vite-env.d.ts` aumentado con el tipo
